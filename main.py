...
from prompts import rag_prompt, email_system_prompt, social_media_post_prompt
...
async def get_response(query: str = Form(...)):
    ...
    response_data = jsonable_encoder(json.dumps({"answer": answer, "source_document": source_document, "doc": doc, 'chat_queries': chat_queries, 'chat_history': chat_responses}))
...
async def get_email_response(project: str = Form(...), name: str = Form(...), info: str = Form(...)):
    ...
    print(f'this is the social post as a response object: {email_response}\n\n')
    return email_response 



@app.get('/social_media_post', response_class=HTMLResponse)
async def social_media_post_page(request: Request):
    return templates.TemplateResponse("social_media_post.html", {"request": request})


@app.post('/generate_social_media_post')
async def generate_social_media_post(query: str = Form(...)):
#async def generate_social_media_post(request: Request):
    print(f'Social media post query: {query} \n')
    social_media_post_generation_chain = LLMChain(llm=llm, prompt=social_media_post_prompt)
    social_media_post_output = social_media_post_generation_chain.run({'user_query': query})
    #print(social_media_post_output)
    
    social_media_output_dict = {'social_media_post_output': social_media_post_output}
    social_media_output_json_string = json.dumps(social_media_output_dict)
    #print(f'\n\n ****  this is the email output dict of the data in json: {social_media_output_json_string}\n\n ')
    social_media_post_output_data = jsonable_encoder(social_media_output_json_string)
    #social_media_post_output_data = jsonable_encoder(social_media_output_dict)
    print(f'\n\n ****  this is the email response of the data in json: {social_media_post_output_data}\n\n ')
    social_media_post_response = Response(social_media_post_output_data)
    print(f'this is the social post as a response object: {social_media_post_response}\n\n')
